...
var canReplay, getCookie, grav, highscore, jump, my_id, pipeHeight, pipeWidth, pipes, playerDead, playerJump, playerScore, players, pos, rot, score, screenClick, setBigScore, setCookie, setHighScore, setMedal, setSmallScore, showScore, showSplash, socket, start, state, tick, update, updateInterval, updatePipes, updatePlayer, vel,
  _this = this;
...
socket.on('update', function(play_arr) {
  var i, _i, _ref, _results;
  players = play_arr;
  _results = [];
  for (i = _i = 0, _ref = players.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
    if (play_arr[i] != null) {
...
update = function() {
  ...
  if (pipes[0] == null) {
    return socket.emit('myevent', pos);
  }
  pipe = pipes[0];
